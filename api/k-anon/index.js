...
const supress = async (str)=>{
    n = str.length
    for(i=0;i<n/2;i++){
      str = setCharAt(str,(Math.floor(Math.random() * n)),"*") 
    }
    return str
}

const random = async ()=>{
    str = left[(Math.floor(Math.random() * left.length))] + '_' +right[(Math.floor(Math.random() * right.length))]
    return str
}

const category = async (age)=>{
     n = Math.ceil(age/10)*10;
    return `< ${n}`
